...
export default function ConfirmationModal({
  ...
  useEffect(() => {
    const myModal = document.getElementById("confirmationModal");

    const handleHiddenEvent = () => {
      cancelCallback();
    };
    myModal?.addEventListener("hidden.bs.modal", handleHiddenEvent);

    return () => {
      myModal?.removeEventListener("hidden.bs.modal", handleHiddenEvent);
    };
  }, [cancelCallback]);
  return (
    <div
